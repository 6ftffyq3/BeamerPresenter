...
SlideLabelWidget::SlideLabelWidget(QWidget *parent) : QWidget(parent)
{
...
void SlideLabelWidget::updateText(const int page) noexcept
{
  ...
  if ((preferences()->global_flags & Preferences::LogSlideChanges) &&
    ...
    std::cout << "Changed page label" << std::setw(9) << string.toStdString()
              << std::setw(4) << edit->text().toStdString() << std::setw(4)
              << new_label.toStdString() << std::endl;
  }
  total->setText(" / " + preferences()->document->pageLabel(
                             preferences()->number_of_pages - 1));
